...
namespace pvfLoaderXinyu
{
    class Pvf
    {
        ...
        */
        ...
        private string ReplaceIfMatch(string input, string compareTo, string replacement)
        {
            // 提取第一个斜杠之前的部分
            string inputPrefix = input.Split('/')[0];
            string compareToPrefix = compareTo.Split('/')[0];

            // 比较并替换
            if (inputPrefix == compareToPrefix)
            {
                return replacement;
            }
